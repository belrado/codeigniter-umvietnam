...
<div class="">
	<div class="comment-sec" id="comment-sec" data-comment-limit="<?=$comment_limit?>">
		<div id="comment-input-box">
			<?php if(($bbs_comment_lv > 1 && $user_lv > 1 && $this->session->userdata('user_id'))) : ?>
			<div class="comment-input-box">
				<div class="comment-header">
					<div>
						<?php if($user_lv > 1 && $user_lv < 7) : ?>
						<?=preg_replace('/@.+$/', '', $this->encryption->decrypt($this->session->userdata('user_email')))?>
						<sup class="user">[<?=fnc_name_change2(fnc_set_htmls_strip($this->session->userdata('user_name')), 6)?>]</sup>
						<?php elseif($user_lv >= 7) : ?>
						<strong class="color_7a0019"><?=$this->session->userdata('user_name')?></strong>
						<sup class="user">[<?=$this->session->userdata('user_nick')?>]</sup>
						<?php endif ?>
					</div>
				</div>
				<div class="comment-body">
					<form action='<?=site_url()?>board/bbs_comment/' method='post' class='BD-check-form comment-register'>
						...
						<div class="comment-textarea">
							<textarea name="bbs_content" class="cbbs_content checkInput check-comment-auth" <?=$c_readonly?> onkeyup="check_content_byte(this, 400, 'limit-txtlength');" placeholder="<?=stripslashes($lang_message['comments_malicious'])?>"><?=$c_comment_txt?></textarea>
			...
			<li>
				<div class="comment-box">
					...
					<?php if($val->user_level > 1 && $val->user_level < 7) : ?>
						<?=preg_replace('/@.+$/', '', $val->user_email)?>
						<sup class="user">[<?=fnc_name_change2(fnc_set_htmls_strip($val->user_name), 6)?>]</sup>
					<?php elseif($val->user_level >= 7) : ?>
						<strong class="color_7a0019"><?=preg_replace('/@.+$/', '', $val->user_name)?></strong>
						<sup class="suser">[<?=fnc_set_htmls_strip($val->user_nick)?>]</sup>
					<?php endif ?>
					</div>
					<div class="comment-list-body">
						<?=nl2br(fnc_set_htmls_strip($val->bbs_content))?>
						<p class="date"><?=set_view_register_time($val->bbs_register, 0, 16, '-')?></p>
					</div>
					...
					<?php if($val->user_id === $this->encryption->decrypt($user_id)) : ?>
						<a href="javascript:;" class="btn-comment-del"
							...
							data-bbs-commentparent="<?=$val->bbs_comment_parent?>"><?=stripslashes($lang_bbs['delete'])?></a><?php endif ?><?php if($user_lv >= 9) : ?>&nbsp;<a href="javascript:;" class="btn-comment-del"
							data-bbs-table="<?=$bbs_table?>"
							data-bbs-id="<?=$val->bbs_id?>"
							data-bbs-iscomment="<?=$val->bbs_is_comment?>"
							data-bbs-parent="<?=$val->bbs_parent?>"
							data-bbs-commentparent="<?=$val->bbs_comment_parent?>"
							data-comment-deltype="super">Del (adm)</a>
					<?php endif ?>
					</div>
				</div>
				<div class="comment-box2 comment-list-body">
					<a href="javascript:;" class="comment-reply-view"
						data-bbs-table="<?=$bbs_table?>"
						data-bbs-id="<?=$val->bbs_id?>"
						data-bbs-iscomment="2"
						data-bbs-parent="<?=$val->bbs_parent?>"
						data-bbs-commentparent="<?=$val->bbs_id?>"><?=stripslashes($lang_bbs['cmt_under_cmt'])?> <span class="comment-num"><?=$val->bbs_comment?></span></a>

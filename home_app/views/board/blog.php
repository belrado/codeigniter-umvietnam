		...
		<div class="bbs-wrapper">
			...
			<?php endif ?>
				...
				<?php if($bbs_result) : ?>
					...
					<li id="blog_box_<?=$val->bbs_id?>" class="item-box <?=($val->bbs_cate==='메디프렙')?'mediprep':(($val->bbs_cate ==='PSU에듀센터')?'psuedu':'blogdefault')?>">
						<?php if($bbs_adm) : ?>
						 ...
						 */ ?>
						<a href="<?=site_url()?>board/view/<?=$bbs_table?>/<?=$val->bbs_id?>/?select=<?=$sch_select?>&amp;keyword=<?=$keyword?>&amp;paged=<?=$paged?>">
								...
								<div class="blog-box2">
									...
									<div class="blog-txt">
										...
										<div class="footer">
											...
											<?php else : ?>
											<div class="blog-logo none"><img src="/assets/img/bbs/psu_logo.png" alt="PSU에듀센터"></div>
											<?php endif ?>
			...
			<?php endif ?>
			<?php if($bbs_total_num > count($bbs_result)) :?>
...
<?php if($this->session->flashdata('bbs_viewid')) : ?>
	var bbs_prev_elemId = "<?=$this->session->flashdata('bbs_viewid')?>";
	if(bbs_prev_elemId){
		var blogBox = $('#blog_box_'+bbs_prev_elemId);
		var elemOffset = blogBox.offset();
		$('html, body').stop(true, true).animate({scrollTop : elemOffset.top-100}, 300);
				...
				if(data.err_msg){
					...
					$.each(data.bbs_list, function(){
						...
						if(adminCheck){
							...
							elem += '<a href="<?=site_url()?>board/modify/<?=$bbs_table?>/'+this.bbs_id+'" class="bbs-admin-modify-btn"><img src="http://mediprep.co.kr/assets/img/bbs/btn_gallery_modify_bt.png" /></a>';
						}
						elem += '<a href="<?=site_url()?>board/view/<?=$bbs_table?>/'+this.bbs_id+'/?select=<?=$sch_select?>&amp;keyword=<?=$keyword?>&amp;paged='+view_paged+'">';
						//if(this.bbs_link===''){
							//elem += '<a href="<?=site_url()?>board/view/<?=$bbs_table?>/'+this.bbs_id+'">';
						//}else{
							//elem += '<a href="'+this.bbs_link+'" target="_blank" data-name="psu">';
						//}
					<?php if($bbs_css_type === 'bbs_typeB') : ?>
